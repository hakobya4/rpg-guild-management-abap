CLASS zcl_rpg_npc_ai DEFINITION
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_npc_details,
             npc_name    TYPE string,
             flavor_text TYPE string,
           END OF ty_npc_details.

    "! Calls the Anthropic Messages API once to generate both a name and
    "! flavor text together
    METHODS generate_npc
      IMPORTING
                iv_npc_race    TYPE string
                iv_npc_role    TYPE string
      RETURNING VALUE(rs_npc)  TYPE ty_npc_details
...
CLASS zcl_rpg_npc_ai IMPLEMENTATION.
  ...
  METHOD generate_npc.
    ...
    IF sy-subrc <> 0 OR lv_api_key IS INITIAL.
      rs_npc-npc_name = 'AI not configured - run ZCL_RPG_SET_AI_KEY first.'.
      RETURN.
    ENDIF.

    DATA(lv_prompt) =
      |Create a character who's race is { iv_npc_race } and who works as a | &&
      |{ iv_npc_role } in a fantasy adventurers' guild town. | &&
      |Reply with ONLY a single JSON object, no markdown code fences, no preamble, | &&
      |with exactly these keys: npc_name (string, just the name), | &&
      |flavor_text (string, a single short vivid paragraph of about 200 characters | &&
      |describing this character - no preamble, just the description itself).|.
    ...
    IF lines( ls_response-content ) = 0.
      ...
      " silently returning an empty NPC.
      rs_npc-npc_name = |[DEBUG] body: { lv_response_json }|.
      RETURN.
    ENDIF.

    DATA(lv_npc_json) = ls_response-content[ 1 ]-text.

    " Remove json wrapping just in case.
    REPLACE ALL OCCURRENCES OF '```json' IN lv_npc_json WITH ''.
    REPLACE ALL OCCURRENCES OF '```'     IN lv_npc_json WITH ''.
    CONDENSE lv_npc_json.

    /ui2/cl_json=>deserialize(
      EXPORTING json = lv_npc_json
      CHANGING  data = rs_npc ).
